...
QBCore.Functions.CreateUseableItem("thermite", function(source, item)
	...
	if Player.Functions.GetItemByName('lighter') ~= nil then
        TriggerClientEvent("qb-bankrobbery:use:thermite", source)
    else
...
QBCore.Functions.CreateUseableItem("laptop", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player.Functions.GetItemByName('laptop') ~= nil then
        TriggerClientEvent("qb-bankrobbery:use:laptop", source)
    end
end)
